...
You can set up a special *RESET* animation to contain the "default pose" for the AnimationPlayer. Add an animation called "RESET", and add tracks for each property that you want to reset. The only keyframe should be at time 0, and give it the desired default values for each track.
